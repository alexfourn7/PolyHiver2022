...
void AjouterFilm(Film film, ListeFilms listeFilms) {
	Film* ptrFilm = &film;
	int capaciteRestante = listeFilms.capacite - listeFilms.nElements;
	if (capaciteRestante == 0) {
		if (listeFilms.capacite == 0)
			listeFilms.capacite = 1;
		listeFilms.capacite *= 2;

		Film** elements = new Film*[listeFilms.capacite];
		for (int i : range(0, listeFilms.nElements)) {
			elements[i] = listeFilms.elements[i];
		}
		elements[listeFilms.nElements] = ptrFilm;
		listeFilms.elements = elements;
		delete[] elements;
...
void EnleverFilm(Film* ptrFilm, ListeFilms listeFilms) {
	...
	for (int i : range(0, listeFilms.nElements)) {
		if (listeFilms.elements[i] != ptrFilm) {
			elements[position++] = listeFilms.elements[i];
		}
	}
	listeFilms.elements = elements;
	delete[] elements;
...
Acteur* TrouverActeur(ListeFilms listeFilms, string nomActeur) {
	span<Film*> films = {listeFilms.elements, size_t(listeFilms.nElements)};
	for (auto film: films) {
		span<Acteur*> acteurs = {film->acteurs.elements, size_t(film->acteurs.nElements)};
		for (auto acteur: acteurs) {
			string nom = acteur->nom;
			if (nom == nomActeur) {
				return acteur;
			}
		}
